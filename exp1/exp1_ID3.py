    ...
    def _predict_sample(self, sample):
        ...
        while not node.is_leaf:
            ...
            if feature_value in node.children:
                ...
                # 如果遇到未知的特征值，返回根节点下最常见的类别
                most_common = self._get_most_common_class(self.root)
                return most_common if most_common is not None else 'sunny'  # 确保返回有效值
        
        return node.value if node.value is not None else 'sunny'  # 确保返回有效值
    
    def _get_most_common_class(self, node):
        """获取节点下最常见的类别"""
        # 递归收集所有叶节点的类别
        def collect_classes(current_node):
            if current_node.is_leaf:
                return [current_node.value]
            else:
                classes = []
                for child in current_node.children.values():
                    classes.extend(collect_classes(child))
                return classes
        
        # 收集所有叶节点的类别
        all_classes = collect_classes(node)
        if not all_classes:
            # 如果没有叶节点，返回默认值（从训练数据中获取）
            return 'sunny'  # 使用训练数据中最常见的类别作为默认值
...
def main():
    ...
    # 由于数据集很小，不划分验证集，使用交叉验证或直接使用简单的预剪枝
    # 构建并训练ID3决策树（使用预剪枝避免过拟合）
    print("\n构建ID3决策树（使用预剪枝）...")
    tree = ID3DecisionTree(max_depth=3, min_samples_split=2)
    ...
    # 在训练集上评估模型
    train_accuracy = tree._calculate_accuracy(X_train, y_train)
    print(f"\n决策树性能:")
    print(f"训练集准确率: {train_accuracy:.4f}")
    
    # 由于数据集太小，不进行后剪枝，避免过度剪枝
    # 直接使用预剪枝的树进行预测
    print("\n对预测数据进行预测...")
...
# 为了演示不同的剪枝效果，添加一个使用交叉验证进行剪枝的函数
def cross_validation_demo():
    """交叉验证演示函数（可选）"""
    train_data, _ = load_data()
    X = train_data.drop(columns=['weather'])
    y = train_data['weather']
    
    # 使用k折交叉验证评估不同参数
    print("\n=== 交叉验证演示 ===")
    print("由于数据集很小，我们尝试不同的预剪枝参数:")
    
    for max_depth in [2, 3, 4]:
        for min_samples in [1, 2]:
            tree = ID3DecisionTree(max_depth=max_depth, min_samples_split=min_samples)
            tree.fit(X, y)
            acc = tree._calculate_accuracy(X, y)
            print(f"最大深度={max_depth}, 最小样本数={min_samples}: 准确率={acc:.4f}")
